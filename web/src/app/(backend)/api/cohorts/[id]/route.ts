...
export async function GET(
  ...
  const cohort = await CohortsService.buscarPorId(params.id);
  if (!cohort) {
    return Response.json({ erro: "Cohort não encontrado" }, { status: 404 });
  }
...
export async function PATCH(
  ...
  try {
    ...
    const atualizado = await CohortsService.atualizar(params.id, dados);
...
export async function DELETE(
  ...
  try {
    await CohortsService.remover(params.id);
