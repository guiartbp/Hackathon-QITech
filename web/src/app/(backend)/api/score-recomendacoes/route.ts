...
export async function GET() {
  const scoreRecomendacoes = await ScoreRecomendacaoService.listar();
  return Response.json(scoreRecomendacoes);
}
...
export async function POST(req: Request) {
  try {
    ...
    const novo = await ScoreRecomendacaoService.criar(dados);
